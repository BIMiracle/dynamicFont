...
@implementation UIButton (KBDynamic)

- (void)updateFontWithContentString:(UIContentSizeCategory)contentString{
    self.titleLabel.font = [self setFontWithStyle:self.fontStyle contentString:contentString weight:[self fontWeight]];
}
...
- (void)setFontStyle:(KBFontSize)fontStyle weight:(UIFontWeight)weight{
    ...
    self.titleLabel.font = [self setFontWithStyle:fontStyle contentString:[[KBDynamicFontManager sharedInstance] getCurrentContentSizeString] weight:weight];
}
